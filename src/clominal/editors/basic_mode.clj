(ns clominal.editors.basic_mode
  (:import (clojure.lang LineNumberingPushbackReader LispReader)
           ...
           (org.fife.ui.rsyntaxtextarea SyntaxConstants TokenTypes RSyntaxUtilities Token SyntaxScheme Style))
...
; Key   -> Target bracket character
; Value -> Vector. The 1st element is correspond bracket character. The 2nd element is 'left side' or 'right side' flag.
(def bracket-pairs {
  \{ [ \} true ]
  \( [ \) true ]
  \[ [ \] true ]
...
(def basic-mode-token-colors {
  SyntaxScheme/RESERVED_WORD               (Style. (Color. 128 128 255))
  SyntaxScheme/RESERVED_WORD_2             (Style. (Color. 128 128 255))
  SyntaxScheme/LITERAL_NUMBER_DECIMAL_INT  (Style. Color/ORANGE)
  SyntaxScheme/LITERAL_NUMBER_FLOAT        (Style. Color/ORANGE)
  SyntaxScheme/LITERAL_NUMBER_HEXADECIMAL  (Style. Color/ORANGE)
  SyntaxScheme/LITERAL_BOOLEAN             (Style. Color/PINK)
  SyntaxScheme/COMMENT_DOCUMENTATION       (Style. (Color. 128 200 128))
  SyntaxScheme/COMMENT_EOL                 (Style. (Color. 128 200 128))
  SyntaxScheme/COMMENT_MULTILINE           (Style. (Color. 128 200 128))
  SyntaxScheme/COMMENT_KEYWORD             (Style. (Color. 128 200 128))
  SyntaxScheme/COMMENT_MARKUP              (Style. (Color. 128 200 128))
  SyntaxScheme/LITERAL_STRING_DOUBLE_QUOTE (Style. (Color. 217 151 47))
  SyntaxScheme/MARKUP_TAG_NAME             (Style. (Color. 128 128 255))
  SyntaxScheme/MARKUP_TAG_ATTRIBUTE        (Style. (Color. 200 100 100))
  SyntaxScheme/MARKUP_TAG_ATTRIBUTE_VALUE  (Style. (Color. 217 151 47))
  SyntaxScheme/OPERATOR                    (Style. (Color. 100 200 100))
  })

(defn apply-basic-mode-colors
  [text-pane]
  (doseq [id (keys basic-mode-token-colors)]
    (.. text-pane (getSyntaxScheme) (setStyle id (basic-mode-token-colors id)))))

;; ------------------------------
;; Initialize Mode
;; ------------------------------
(defn init-mode
  [text-pane mode-name mode]
  (println (format "[%s] Loading ..." mode-name))
  ; keybinds
  (let [settings {
